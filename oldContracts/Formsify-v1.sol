...
contract Formsify {
    ...
    /**
     * @dev
     * If form is public, publicKey will be set,
    ...
    // ============================
    // OWNER MODIFICATION FUNCTIONS
    // ============================

    /**
     * @dev Modify form's start time
     * @param _newStartTime New start time of form
     */
    function modifyStartTime(uint256 _newStartTime) public {
        require(msg.sender == owner, "Only owner can modify start time!");
        startTime = _newStartTime;
    }

    /**
     * @dev Modify form's end time
     * @param _newEndTime New end time of form
     */
    function modifyEndTime(uint256 _newEndTime) public {
        require(msg.sender == owner, "Only owner can modify end time!");
        endTime = _newEndTime;
    }

    /**
     * @dev Modify form's max fills
     * @param _newMaxFills New max fills of form
     */
    function modifyMaxFills(uint256 _newMaxFills) public {
        require(msg.sender == owner, "Only owner can modify max fills!");
        maxFills = _newMaxFills;
    }

    /**
     * @dev Modify allowed total fills by an address
     * @param _newAllowedTotalFills New allowed total fills of form
     */
    function modifyAllowedTotalFills(uint256 _newAllowedTotalFills) public {
        require(
            msg.sender == owner,
            "Only owner can modify allowed total fills!"
        );
        allowedTotalFills = _newAllowedTotalFills;
    }
